...
window.dom = {
  create: function create(string) {
    ...
    var container = document.createElement('template'); // 要trim，防止拿到空字符

    container.innerHTML = string.trim(); // 必须要 使用 .content  要不然拿不到

    return container.content.firstChild; // 或者
  ...
  append: function append(newNode, node) {
    return newNode.appendChild(node);
  },
  wrap: function wrap(node, newNode) {
    // 先把newNode放在node节点的前面   后面也行
    dom.before(node, newNode); // 然后把node节点放在newNode里面

    dom.append(newNode, node);
  },
  remove: function remove(node) {
    node.parentNode.removeChild(node); //返回删除的节点
...
},{}]},{},["F:/nodejs/node_modules/parcel/src/builtins/hmr-runtime.js","dom.js"], null)
